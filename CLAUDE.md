...
2. **Native locations unless the configuration declares otherwise.** Never point
   ...
   is what keeps the guard's reach total, and it is proved of each such file
   rather than assumed. The shell-init snippet is the one file bx generates
   under it: fixed text from bx's source that sets no environment variable
   outside bx's own `BX_` namespace, and gets every other variable by sourcing
   a guarded environment fragment. Declare no root and nothing moves: no
   location, no list of locations and no anchor is permitted at all. A program,
   a search list and a socket say what a tool runs, where it looks and what it
   connects to rather than where its files live, so they move nothing and need
   no root — but no more than any other kind may they point inside a directory
   bx owns. Containment is decided lexically, never by touching the filesystem,
