...
///
/// bash's interactive file is judged the same way: by its `env` phase, its
/// one environment fragment, and the history file it names. Its alias lines
...
mod tests {
    ...
    mod env_placement {
        ...
        fn path_entries_land_once_in_the_zshenv_fragment_after_its_variables() {
            ...
            // And nowhere else: bash's file carries the variables, not the
            // PATH lines, which are written in zsh's words.
            for rel in [
                ".config/environment.d/50-bx.conf",
                ".local/share/bx/zshrc.zsh",
                ".local/share/bx/bashrc.bash",
